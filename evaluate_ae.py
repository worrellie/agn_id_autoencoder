...
if os.path.exists(test_res_path):
    ...
    test_loader = torch.utils.data.DataLoader(
        test, batch_size=1, shuffle=True, num_workers=num_workers
    )
    ...
    with torch.no_grad():
        for x, x_mask in test_loader:
            ...
            reconstruction_loss = funcs.loss_calc_per_spec(
                x_hat,
                x,
                x_mask,
            )
            ...
            test_results["recon_losses"].append(
                reconstruction_loss.cpu().squeeze().float().item()
            )
