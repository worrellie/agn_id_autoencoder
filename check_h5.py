...
def check_h5(file):
    ...
    with h5py.File(H5_FILE, "r") as hf:
        for split in splits:
            ...
            snr_data[split] = snrs
            print(
                f"{split:12s} | n={len(snrs):4d} | mean SNR = {np.mean(snrs):.3f} | median = {np.median(snrs):.3f} | std = {np.std(snrs):.3f}"
            )
    ...
    for (split, snrs), color in zip(snr_data.items(), colors):
        ax.hist(
            ...
            bins=30,
            alpha=0.6,
            label=f"{split} (mean={np.mean(snrs):.2f})",
            color=color,
            edgecolor="white",
        )
...
with h5py.File("test_all_spectra_sf_q.h5", "r") as hf:
    ...
    for i in random_idxs:
        ...
        mask = s != 0
        print(
            f"  spec {i}: full mean={s.mean():.6f}, unmasked mean={s[mask].mean():.6f}"
        )
